...
export default class Config {
  ...
  static async env(args: ConfigArgs): Promise<Config> {
    ...
    if (!process.env.StackName || process.env.StackName === "test") {
      ...
      if (args.local)
        throw new Error(
          "local option cannot be used in production mode - Set StackName=test"
        );
      ...
      if (!process.env.PMTILES_URL)
        throw new Error("PMTILES_URL env must be set");
      if (!process.env.ASSET_BUCKET)
        throw new Error("ASSET_BUCKET env must be set");
    ...
    const pg: Pool<typeof pgtypes> = await Pool.connect(
      args.postgres,
      pgtypes,
      {
        ssl:
    ...
    const config = new Config({
      ...
      TileBaseURL: process.env.TileBaseURL
        ? new URL(process.env.TileBaseURL)
        : new URL("./data-dev/zipcodes.tilebase", import.meta.url),
    ...
    if (process.env.SubnetPublicA)
      config.SubnetPublicA = process.env.SubnetPublicA;
    if (process.env.SubnetPublicB)
      config.SubnetPublicB = process.env.SubnetPublicB;
    if (process.env.MediaSecurityGroup)
      config.MediaSecurityGroup = process.env.MediaSecurityGroup;
  ...
  static async fetchSigningSecret(StackName: string): Promise<string> {
    const secrets = new SecretsManager.SecretsManagerClient({
