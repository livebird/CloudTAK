...
export default class AuthProvider {
  ...
  async login(username: string, password: string): Promise<string> {
    ...
    const url = new URL(
      "/oauth/token",
      this.config.local ? `http://localhost:${port}` : this.config.MartiAPI
    );
    ...
    if (!authres.ok) {
      throw new Err(
        500,
        new Error(`Status: ${authres.status}: ${await authres.text()}`),
        "Non-200 Response from Auth Server - Token"
      );
    }
    ...
    if (
      ...
      body.error_description.startsWith("Bad credentials")
    ) {
    ...
    const split = Buffer.from(body.access_token, "base64")
      .toString()
      .split("}")
      .map((ext) => {
        return ext + "}";
  ...
  async valid(
    ...
    try {
      ...
      if (
        moment(validTo, "MMM DD hh:mm:ss YYYY").isBefore(
          moment().add(7, "days")
        )
      ) {
      ...
      if (password) {
        const api = await TAKAPI.init(
          new URL(this.config.MartiAPI),
          new APIAuthPassword(profile.username, password)
        );
