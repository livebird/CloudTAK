...
try {
  ...
  process.env = Object.assign(
    JSON.parse(String(fs.readFileSync(dotfile))),
    process.env
  );
} catch (err) {
...
if (import.meta.url === `file://${process.argv[1]}`) {
  const config = await Config.env({
    ...
    postgres:
      ...
      args.postgres ||
      "postgres://postgres@localhost:5432/tak_ps_etl",
...
export default async function server(config: Config) {
  ...
  await schema.load(new URL("./routes/", import.meta.url), config, {
  ...
  const wss = new WebSocketServer({
    ...
    try {
      ...
      if (!params.get("connection"))
        throw new Error("Connection Parameter Required");
      ...
      if (!config.wsClients.has(parsedParams.connection))
        config.wsClients.set(parsedParams.connection, []);

      if (!config.conns)
        throw new Error("Server not configured with Connection Pool");
      ...
      if (!isNaN(parseInt(parsedParams.connection))) {
        ...
        auth instanceof AuthUser &&
        parsedParams.connection === auth.email
      ) {
        let client;
        if (!config.conns.has(parsedParams.connection)) {
          const profile = await config.models.Profile.from(
            parsedParams.connection
          );
          if (!profile.auth.cert || !profile.auth.key)
            throw new Error("No Cert Found on profile");

          client = await config.conns.add(
            new ProfileConnConfig(
              config,
              parsedParams.connection,
              profile.auth
            ),
        ...
        const connClient = new ConnectionWebSocket(
          ws,
          parsedParams.format,
          client
        );
  ...
  return new Promise((resolve) => {
    ...
    const srv = app.listen(port, () => {
      if (!config.silent) console.log(`ok - http://localhost:${port}`);
      return resolve(srv);
    });
